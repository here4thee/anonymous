...
contract WeiCards {
        ...
        returns (bool success)
    {
        ...
        uint256 sellerProceeds = msg.value - auctioneerCut; // underflow
        // Credit seller balance
        pendingWithdrawals[_seller] += sellerProceeds; // overflow
        // Credit auctionner balance
        pendingWithdrawals[_auctioneer] += auctioneerCut; // overflow
        return true;
    }
    ...
    function _computeCut(uint256 _price, uint256 _cut) internal pure
        returns (uint256)
    {
        return _price * _cut / 10000; // overflow
    }
}