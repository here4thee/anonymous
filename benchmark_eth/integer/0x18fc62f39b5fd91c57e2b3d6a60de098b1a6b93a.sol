...
contract Distributor {
    ...
    for(uint i = 0; i < _receivers.length; i++) {
      ...
      if(balance > 0) {
        if(isContract(receiver)) {
          ...
          stopAddresses[receiver] += balance; // overflow
          stopAddressesTotal += balance; // overflow
        } else {
          if(balances[receiver] == 0) receivers.push(receiver); 
          balances[receiver] += balance; // overflow
  ...
  function changeBalance(address to, uint newValue) public onlyOwner {
    ...
    total -= balances[to];  // underflow
    balances[to] = newValue;
    total += newValue; // overflow
  }
