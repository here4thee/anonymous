...
contract PowhCoin4 {
    ...
    function withdraw(uint tokenCount) public returns (bool) {
        ...
        payouts[msg.sender] += (int256) (balance * PRECISION); // overflow // overflow
        totalPayouts += (int256) (balance * PRECISION); // overflow // overflow
    ...
    function transferTokens(address _from, address _to, uint256 _value) internal {
        ...
        if (_to == address(this)) {
            ...
            int256 payoutDiff = (int256) (earningsPerShare * _value); // overflow
            balanceOfOld[_from] -= _value;
            balanceOfOld[_to] += _value; // overflow
            payouts[_from] -= payoutDiff; // underflow
            payouts[_to] += payoutDiff;  // overflow
        }
    ...
    function dividends(address _owner) public constant returns (uint256 amount) {
        return (uint256) ((int256)(earningsPerShare * balanceOfOld[_owner]) - payouts[_owner]) / PRECISION;  // overflow // underflow
    }
    ...
    function balance() internal constant returns (uint256 amount) {
        return this.balance - msg.value; // underflow
    }

    function reserve() public constant returns (uint256 amount) {
        return balance() - ((uint256) ((int256) (earningsPerShare * totalSupply) - totalPayouts) / PRECISION) - 1; // overflow // underflow
    }
    ...
    function buy() internal {
        ...
        var buyerfee = fee * PRECISION; // overflow
        if (totalSupply > 0) {
            ...
            var holderfee = fee * holderreward; // overflow
            buyerfee -= holderfee;
        ...
        totalSupply += numTokens; // overflow
        // add numTokens to balance
        balanceOfOld[sender] += numTokens; // overflow
        // fix payouts so that sender doesn't get old earnings for the new tokens.
        // also add its buyerfee
        var payoutDiff = (int256) ((earningsPerShare * numTokens) - buyerfee); // overflow // underflow
        payouts[sender] += payoutDiff; // overflow
        totalPayouts += payoutDiff; // overflow
    }
    ...
    function sell(uint256 amount) internal {
        ...
        balanceOfOld[msg.sender] -= amount; // underflow

        // fix payouts and put the ethers in payout
        var payoutDiff = (int256) (earningsPerShare * amount + (numEthers * PRECISION));  // overflow // overflow // overflow
        payouts[msg.sender] -= payoutDiff; // underflow
        totalPayouts -= payoutDiff; // underflow
    }

    function getTokensForEther(uint256 ethervalue) public constant returns (uint256 tokens) {
        return fixedExp(fixedLog(reserve() + ethervalue)*CRRN/CRRD + LOGC) - totalSupply; // overflow // underflow
    }

    function getEtherForTokens(uint256 tokens) public constant returns (uint256 ethervalue) {
        if (tokens == totalSupply)
            return reserve();
        return reserve() - fixedExp((fixedLog(totalSupply - tokens) - LOGC) * CRRD/CRRN); // underflow
    }
