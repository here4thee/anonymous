...
library DirectCrowdsaleLib {
                           ...
                           returns (bool)
  {
    ...
    _amount = _amount - _remainder; // underflow
    self.base.leftoverWei[msg.sender] += _leftoverWei; // overflow
...
library CrowdsaleLib {
    ...
    if (msg.sender == self.owner) {
      if(!crowdsaleEnded(self)){
        ...
        if(self.percentBurn > 0){
          uint256 _burnAmount = (self.withdrawTokensMap[msg.sender] * self.percentBurn)/100; // overflow
          self.withdrawTokensMap[msg.sender] = self.withdrawTokensMap[msg.sender] - _burnAmount; // underflow
  ...
  function getTokensSold(CrowdsaleStorage storage self) public view returns (uint256) {
    return self.startingTokenBalance - self.withdrawTokensMap[self.owner]; // underflow
  }
}
...
library TokenLib {
                        ...
                        returns (bool)
  {
    ...
    self.balances[_to] = self.balances[_to] + _value;  // overflow
  ...
  function mintToken(TokenStorage storage self, uint256 _amount) public returns (bool) {
    ...
    self.balances[self.owner] = self.balances[self.owner] + _amount;  // overflow
  ...
  function burnToken(TokenStorage storage self, uint256 _amount) public returns (bool) {
      ...
      self.totalSupply = self.totalSupply - _amount; // underflow
