...
contract BlocklancerToken is ERC20Interface {
	...
	function addToken(address invest,uint256 value){
		if(msg.sender!=master)throw;
		balances[invest]+=value; // overflow
		totalTokens+=value; // overflow
	}
    ...
    function() payable external {
        ...
        var numTokens = msg.value * getExchangeRate(msg.sender); // overflow
    ...
    function finalize(uint percentOfTotal) external {
        ...
        uint256 additionalTokens = totalTokens * percentOfTotal / (100 - percentOfTotal); // overflow
