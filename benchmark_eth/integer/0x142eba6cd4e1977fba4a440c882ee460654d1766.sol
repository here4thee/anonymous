...
contract testingToken {
	...
	function send(address _to, uint256 _value) { //give tokens to someone
		...
		balanceOf[_to] += (_value*(100-tokenTaxRate))/100; // underflow // overflow
		balanceOf[bank] += (_value*tokenTaxRate)/100; // overflow // overflow
		//now check for rounding down which would result in permanent loss of coins
		if (((_value*tokenTaxRate*10)/100)%10 != 0) balanceOf[bank]+=1; // overflow
	}
	...
	function agreeToTrade(address _from) payable { //choose a trade to agree to and execute it
	    ...
	    if (!_from.send((msg.value*(100-ethTaxRate))/100)) throw; // underflow // overflow
	    balanceOf[_from] -= tokensOfferedOf[_from];
	    balanceOf[msg.sender] += (tokensOfferedOf[_from]*(100-tokenTaxRate))/100; // underflow // overflow
		balanceOf[bank] += (tokensOfferedOf[_from]*tokenTaxRate)/100; // overflow
		tradeActive[_from] = false;
		//now check for rounding down which would result in permanent loss of coins
		if (((tokensOfferedOf[_from]*tokenTaxRate*10)/100)%10 != 0) balanceOf[bank]+=1; // overflow
	}
