...
contract MonethaBuyer {
  ...
  function withdraw(address user){
    ...
    // Withdraw the user's tokens if the contract has purchased them.
      ...
      uint256 tokens_to_withdraw = (balances[user] * contract_token_balance) / contract_eth_value; // overflow
      // Update the value of tokens currently held by the contract.
      contract_eth_value -= balances[user]; // underflow
  ...
  function add_to_withdraw_bounty() payable {
    ...
    require(msg.sender == developer);
    // Update bounty to include received amount.
    withdraw_bounty += msg.value; // overflow
  }
  ...
  function claim_bounty(){
    ...
    contract_eth_value = this.balance - (claimed_bounty + withdraw_bounty); // underflow
