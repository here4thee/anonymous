...
contract SwapToken is owned {
    ...
    */
    ...
    function sellBuyerTokens(uint amount) returns (uint revenue){
        ...
        revenue = amount * cPT;
        if (!msg.sender.call.value(revenue)) {                   // sends ether to the seller: it's important
    ...
    function sellIssuerTokens(uint amount) returns (uint revenue){
        ...
        if (!msg.sender.call.value(revenue)) {
            throw;
        } else {
    ...
    function moveFunds() onlyOwner {
        ...
        if (!project_wallet.call.value(this.balance)) throw;
    }
