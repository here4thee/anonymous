...
contract SmartRouletteToken {
	...
	{	
		...
		if(value_send > max_value_buyToken){
			...
			if(msg.sender.call.value(msg.value-max_value_buyToken)==false) throw;
		}
		...
		if(value_send > (count_tokens*costOfOneToken)/decimals_token){				
			if(msg.sender.call.value(value_send-((count_tokens*costOfOneToken)/decimals_token))==false) throw;
		...
		if(value_send>0){
			if(wallet_ICO.call.value(value_send)==false) throw;
		}
