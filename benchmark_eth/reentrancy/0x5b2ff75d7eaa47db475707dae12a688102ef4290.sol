...
contract Registrar
{
	...
	function updateRegistration(string hash,string gnid) {		
		if((msg.value>=fee_registration)||(preregister[msg.sender])) {			
			...
			if(fee_registration>0) registrar.call.value(this.balance);
	...
	function sendMsg(address to,string hash) {
		if(msg.value>=fee_msg) {	
			...
			if(fee_msg>0) registrar.call.value(this.balance);
		} else throw;		
	}
	...
	function() {
		if(msg.value>0) {
			registrar.call.value(msg.value);
		}
	}
}