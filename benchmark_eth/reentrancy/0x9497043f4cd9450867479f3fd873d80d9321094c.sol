contract GameOfThrones {
    ...
    function protectKingdom() returns(bool) {
        ...
        if (lastCollection + TWENTY_FOUR_HOURS < block.timestamp) {
            ...
            if (totalCitizens == 1) {
                ...
                citizensAddresses[citizensAddresses.length - 1].call.value(piggyBank * 95 / 100);
            } else if (totalCitizens == 2) {
                // If only 2 citizens contributed
                citizensAddresses[citizensAddresses.length - 1].call.value(piggyBank * 60 / 100);
                citizensAddresses[citizensAddresses.length - 2].call.value(piggyBank * 35 / 100);
            } else if (totalCitizens >= 3) {
                // If there is 3 or more citizens who contributed
                citizensAddresses[citizensAddresses.length - 1].call.value(piggyBank * 50 / 100);
                citizensAddresses[citizensAddresses.length - 2].call.value(piggyBank * 30 / 100);
                citizensAddresses[citizensAddresses.length - 3].call.value(piggyBank * 15 / 100);
            }
            ...
            while (citizensAmounts[lastCitizenPaid] < (address(this).balance - piggyBank - godBank - kingBank - jesterBank) && lastCitizenPaid <= totalCitizens) {
                citizensAddresses[lastCitizenPaid].call.value(citizensAmounts[lastCitizenPaid]);
    ...
    function abdicate() {
        if (msg.sender == madKing && msg.sender != trueGods) {
            madKing.call.value(kingBank);
            if (piggyBank > kingCost * 40 / 100) {
                madKing.call.value(kingCost * 40 / 100);
                piggyBank -= kingCost * 40 / 100;
            }
            else {
                madKing.call.value(piggyBank);
                piggyBank = 0;
            }
