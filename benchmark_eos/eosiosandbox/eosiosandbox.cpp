...
class eosiosandbox : public eosio::contract {
	
  const uint64_t INIT_BATCH = 100; // 每次分红处理人数
  const uint64_t INIT_WEIGHT = 7*24*60*60; // 权重奖池领取时间（7天）
  const uint64_t INIT_TIME = 1534594088; // 2018-08-18 20:08:08 启动游戏
  const uint64_t INIT_EOS = 100000000.0;//底仓股份1万，用于bancor定价，不能交易
  const uint64_t INIT_KEY = 10000000000.0;//总股本100万
  const std::string TEAM_ACCOUNT = "imyogowallet";//研发团队账号

  public:
    eosiosandbox(account_name self):eosio::contract(self),users(_self, _self),games(_self, _self),bonuses(_self, _self){}
    ...
    void transfer( account_name from, account_name to, asset quantity, std::string memo ) {
        ...
        ////eosio_assert( now()>=1534248488, "游戏在2018年8月14日下午5点30内测");    
        //eosio_assert( now()>=INIT_TIME, "游戏在2018年8月18日晚上8点8分8秒启动游戏");  
        ...
        if(quantity.is_valid() && quantity.symbol == S(4, EOS) && from != _self && to == _self)
        {
            if(quantity.amount==1)
            {
                ...
                std::make_tuple(_self,from, balance, std::string("沙盒游戏团队感谢你的支持：https://yougou.io"))
                ).send();
...
EOSIO_ABI_EX(eosiosandbox, (transfer)(sell)(jackpot))
