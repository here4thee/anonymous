...
void eosgameworld::transfer(account_name from, account_name to, asset quantity, string memo) {
    ...
    st_number number = get_number();
    number.numbers = number.numbers + 1;
    if (time_point_sec(now() + gap_delta) < round.end) {
        round.end = max(time_point_sec(now()) + gap - (number.numbers * gap_delta), time_point_sec(now())+gap_delta);
    }

    sgt_number.set(number, _self);
...
void eosgameworld::withdraw(account_name to, asset quantity) {
    ...
    // set quantity
    //eosio_assert(quantity.amount + vault > quantity.amount, "integer overflow adding withdraw balance");
