...
namespace eosdac {
    ...
    void eosdactokens::newmemterms(string terms, string hash) {
        ...
        if (memberterms.begin() != memberterms.end()) {
            ...
            //eosio_assert(!(terms == last->terms && hash == last->hash), "ERR::NEWMEMTERMS_DUPLICATE_TERMS::Next member terms cannot be duplicate of the latest.");
        }
    ...
    void eosdactokens::memberunreg(name sender) {
        ...
        if (is_account(configs().notifycontr)) {
            name notifyContract = configs().notifycontr;
            ...
            if (candidateidx != candidatesTable.end()) {
                ...
                //eosio_assert(candidateidx->is_active != 1, "ERR::MEMBERUNREG_ACTIVE_CANDIDATE::An active candidate must resign their nomination as candidate before being able to unregister from the members.");
            }
        }
